...
public class PressingCategory extends CreateRecipeCategory<PressingRecipe, PressingDisplay> {
	...
	public List<Widget> setupDisplay(PressingDisplay display, Rectangle bounds) {
		...
		widgets.add(Widgets.createSlot(new Point(origin.x + 26, origin.y + 50)).disableBackground().markInput().entries(display.getInputEntries().get(0)));
		widgets.add(WidgetUtil.textured(AllGuiTextures.JEI_SLOT, origin.x + 26, origin.y + 49));
		...
		for (int outputIndex = 0; outputIndex < results.size(); outputIndex++) {
			...
			widgets.add(Widgets.createSlot(new Point((origin.x + 131 + 19 * outputIndex) + 1, origin.y + 50 + 1)).disableBackground().markOutput().entry(EntryStack.of(VanillaEntryTypes.ITEM, results.get(outputIndex).getStack()).tooltip(tooltip)));
